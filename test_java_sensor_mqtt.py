#!/usr/bin/env python3
# test_java_sensor_mqtt.py - Teste para comandos MQTT do sensor Java
...
if len(sys.argv) > 1:
    ...
    DEVICE_ID = "temp_sensor_mqtt_001"  # Padrão para sensor Java
...
class JavaSensorMQTTTester:
    def __init__(self, device_id):
        ...
        self.client = mqtt.Client(f"java_sensor_tester_{int(time.time())}")
    ...
    def send_command(self, command_type, command_value=""):
        request_id = f"java_test_{int(time.time() * 1000)}"
        command = {
    ...
    def run_tests(self):
        logger.info("Iniciando testes do Sensor Java MQTT...")
        if not self.connect():
